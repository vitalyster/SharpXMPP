...
namespace SharpXMPP
{
    ...
    public static class DNS
    {
        private static IPAddress[] GetDnsAddresses()
        {
            ...
            Array.ForEach(NetworkInterface.GetAllNetworkInterfaces(), iface =>
                                                                          {
                                                                              if (iface.OperationalStatus == OperationalStatus.Up)
                                                                              {
                                                                                  result.AddRange(iface.GetIPProperties().DnsAddresses.
                                                                                      ToArray());
                                                                              }
                                                                          });
        ...
        public static List<SRVRecord> ResolveXMPPClient(string domain)
        {
        ...
        private static byte[] EncodeQuery(string domain)
        {
            ...
            Array.ForEach(buff.ToString().Split('.'), writer.Write);
